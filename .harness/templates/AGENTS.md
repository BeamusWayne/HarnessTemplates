...
> 本文件是本项目 agent 指令的**唯一事实来源（canonical）**。Claude Code 通过 `CLAUDE.md` 里的 `@AGENTS.md` import 读取它；Codex / Cursor / Gemini CLI 等直接读本文件。改规则只改这里。
...
### Rule 6 — Token 预算要有意识，但不要被卡死
- 参考预算（数量级，不是硬红线）：单个功能子任务约 30,000 tokens，单会话约 200,000 tokens。任务确实复杂时可以超出，但要在报告里显式说明超了多少、为什么。
- 每完成一个功能的子任务后，简要报告剩余预算的粗略估算。
- 当剩余预算 < 25% 时：暂停当前功能，更新 claude-progress.md（记录进度和重启路径），提示用户"可考虑开启新会话继续"。
- 自治模式下，剩余预算 < 15% 时：完成当前功能的收尾流程后停止，不再开启新功能。
- 宁可显式报告超支，也不要静默超限——更不要为了省 token 而跳过验证或削弱测试。
...
REPEAT (max: autonomous_config.max_iterations_per_feature 次，默认 5):
  1. 实现或修复代码
  2. 运行 feature.verification 描述的验证步骤（或 ./init.sh verify）
  ...
  6. 如果 iteration_count >= max_iterations_per_feature → 标记 blocked，记录 blocked_reason，退出
...
满足以下任一条件时，停止自治循环：
- 连续 2 个功能变为 blocked
- 当前功能的 iteration_count 达到 max_iterations_per_feature
- 运行 ./init.sh health 失败（基础环境坏了）
- Token 预算剩余不足 25%（提醒用户）；不足 15%（强制停止）
