...
自治循环协议。定义自治模式的触发条件、循环逻辑（外层功能循环 / 中层验证-修复循环 / 内层构建-测试循环）、退出条件、升级条件表、循环重复检测、回滚保护，以及自治工作总结报告模板。

**怎么用：**

- 复制到项目根目录
- 当项目需要自治迭代模式时，agent 在进入自治模式前读取此文件
- 配合 `self-eval-trigger.md` 和 `autonomous_config`（在 `feature_list.json` 中）使用

### self-eval-trigger.md

自我评审触发协议。定义自治模式下的评审触发时机（passing 前、blocked 前、连续 3 个完成后）、评审流程（读取 rubric、六维度打分、阈值判断）、评分校准提醒、评审结果记录。

**怎么用：**

- 复制到项目根目录
- 自治模式下，agent 在标记功能状态前自动触发评审
- 评审分数记录到 `claude-progress.md` 的自治迭代记录中

### plan-template.md

执行计划模板。复杂任务开始前，用独立文件记录目标、步骤、风险和验证标准。

**怎么用：**

- 存放于 `.harness/templates/` 目录
- 创建新计划：`harness new-plan <任务简述>`
- 计划完成后，手动移到 `.harness/plans/completed/`

### history-template.md

变更历史模板。重要变更按标准格式记录，按月归档。

**怎么用：**

- 存放于 `.harness/templates/` 目录
- 创建新记录：`harness new-history <任务简述>`
- 记录保存在 `.harness/histories/YYYY-MM/`

## init.sh 子命令

init.sh 已从线性脚本升级为子命令模式：

| 命令 | 用途 |
|------|------|
| `./init.sh` | 默认：装依赖 + 跑验证 |
| `./init.sh health` | 环境健康检查（端口、验证） |
| `./init.sh verify` | 只跑功能验证 |
...
串联所有检查：结构检查 + 健康检查 + 功能验证。供 CI 或自治循环调用。
