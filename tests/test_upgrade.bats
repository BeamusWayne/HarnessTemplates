...
@test "harness upgrade updates scripts" {
  "$HARNESS_BIN" init --local
  echo "# old version" > .harness/scripts/check-harness.sh
  run "$HARNESS_BIN" upgrade --local --auto
  assert_exit_code 0
  assert_output_contains "script"
}

@test "harness upgrade updates hook-guard.sh" {
  "$HARNESS_BIN" init --local
  echo "# old" > .harness/scripts/hook-guard.sh
  run "$HARNESS_BIN" upgrade --local --auto
  assert_exit_code 0
  assert_file_contains ".harness/scripts/hook-guard.sh" "post-edit"
}

@test "harness upgrade updates git pre-commit hook" {
  git init
  "$HARNESS_BIN" init --local
  echo "# managed by harness — old version" > .git/hooks/pre-commit
  run "$HARNESS_BIN" upgrade --local --auto
  assert_exit_code 0
  assert_file_contains ".git/hooks/pre-commit" "feature_list.json"
}

@test "harness upgrade --dry-run shows diff for changed files" {
  "$HARNESS_BIN" init --local
  echo "MODIFIED CONTENT" >> CLAUDE.md
  run "$HARNESS_BIN" upgrade --local --dry-run
  assert_exit_code 0
  assert_output_contains "MODIFIED CONTENT"
}
